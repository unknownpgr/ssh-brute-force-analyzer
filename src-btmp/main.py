...
while True:
    entries = get_btmp()
    print("Number of entries:", len(entries), flush=True)
    cur.executemany("INSERT OR IGNORE INTO btmp VALUES(?,?,?,?)", entries)
    con.commit()
    cur.execute("SELECT COUNT(*) FROM btmp")
    count = cur.fetchall()[0][0]
    print("Number of entities:", count, flush=True)

    # Run for every 12 hour
    time.sleep(12 * 3600)
